...
#'
#' @return A tibble which is automatically stored in top_tweet.rda
#' @export
#'

top <- function(q,lim,rDrive){
  ...
  Sys.sleep(7)
  snap <- rDrive$findElement(using = "xpath", value = "//span[contains(text(),'Top')]")
  snap$clickElement()
  Sys.sleep(7)
  df.user <- c()
  df.time <- c()
